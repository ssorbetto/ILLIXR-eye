...
class offline_eye : public threadloop {
public:
    offline_eye(const std::string& name_, phonebook* pb_)
        : threadloop{name_, pb_}
        , sb{pb->lookup_impl<switchboard>()}
        , _m_eye_publisher{sb->get_writer<eye_type>("eye")}
        , _m_sensor_data{load_data()}
        , dataset_first_time{_m_sensor_data.cbegin()->first}
        , last_ts{0}
        , _m_rtc{pb->lookup_impl<RelativeClock>()}
        , next_row{_m_sensor_data.cbegin()} {
        spdlogger(std::getenv("offline_eye_LOG_LEVEL"));
    }
    ...
    void _p_one_iteration() override {
...
#endif
            // Handling the last eye images. There's no more rows after the nearest_row, so we set after_nearest_row
        ...
        if (last_ts != nearest_row->first) {
            ...
            auto img0 = nearest_row->second.eye0.load();
            auto img1 = nearest_row->second.eye1.load();
            ...
            _m_eye_publisher.put(_m_eye_publisher.allocate<eye_type>(eye_type{
...
private:
    ...
    switchboard::writer<eye_type>                  _m_eye_publisher;
